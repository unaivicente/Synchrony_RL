...
for (h in 1:2){
  ...
  write.csv(output, sprintf('NEW2Lvl1_%s_%s.csv', dty$a[h], fdf$a[k]), row.names = FALSE)
  ...
  bayes <- brm(est|se(ste) ~ 1 + (1|con), family=gaussian, data=output, cores=number_of_cores_to_use, inits='0', iter=6000, 
               chains=4, warmup=2000, control=list(adapt_delta=0.9), prior = bprior)
  pos <- as.draws(bayes)

  write.csv(pos, sprintf('NEW2Lvl2_%s_%s.csv', dty$a[h], fdf$a[k]), row.names = FALSE)
  rm(distinct_pairs)
  }
}
