...
for (h in 1:2){
  for (k in 1:4){
    postsamp <- read.csv(sprintf('NEWAdjH_Lvl2_%s_%s.csv', dty$a[h], fdf$a[k]), sep=',', header=TRUE)
    firstlev <- read.csv(sprintf('NEWAdj_Lvl1_%s_%s.csv', dty$a[h], fdf$a[k]), sep=',', header=TRUE)
    ...
    for(i in 1:(nrow(con))){
      ...
      hyp1test <- equivalence_test(var_hyp1, ci=(.95), range=c(-.01,.01)*sd(firstlev$estH))
    ...
    write.csv(output, sprintf('95_01_NEWAdjH_Results_%s_%s.csv', dty$a[h], fdf$a[k]), row.names = FALSE)
  }
}
